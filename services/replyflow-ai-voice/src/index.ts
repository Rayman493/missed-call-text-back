...
async function finalizeCompleteIntakeOnce(
  ...
  try {
    ...
    console.log('[AI SUMMARY SMS DELEGATION] =========================================');
    console.log('[AI SUMMARY SMS DELEGATION] SMS formatting and sending delegated to main app');
    console.log('[AI SUMMARY SMS DELEGATION] AI voice service only stores extracted data');
    console.log('[AI SUMMARY SMS DELEGATION] Main app uses centralized formatter');
    console.log('[AI SUMMARY SMS DELEGATION] Timestamp:', new Date().toISOString());
    console.log('[AI SUMMARY SMS DELEGATION] =========================================');

    console.log('[FINALIZE COMPLETE STEP 4] =========================================');
    console.log('[FINALIZE COMPLETE STEP 4] Completed: Extracted data stored');
    console.log('[FINALIZE COMPLETE STEP 4] SMS sending delegated to main app');
...
function sendStagePrompt(
  ...
  if (!callSessionState.assistantSpeaking) {
    ...
    console.log('[ASSISTANT SPEAKING TRUE - BEFORE RESPONSE CREATE] Previous state:', callSessionState.assistantSpeaking);
    console.log('[ASSISTANT SPEAKING TRUE - BEFORE RESPONSE CREATE] Stage:', stage);
    console.log('[ASSISTANT SPEAKING TRUE - BEFORE RESPONSE CREATE] Source: sendStagePrompt (before OpenAI send)');
    ...
    (twilioHandler as any).lastPromptStage = stage;

    console.log('[CALLER AUDIO BLOCKED - AI SPEAKING] =========================================');
    console.log('[CALLER AUDIO BLOCKED - AI SPEAKING] Caller audio blocked - AI is about to send prompt');
    console.log('[CALLER AUDIO BLOCKED - AI SPEAKING] Stage:', stage);
  ...
  console.log('[SCRIPTED PROMPT SELECTED] =========================================');
  console.log('[SCRIPTED PROMPT SELECTED] Stage:', stage);
  console.log('[SCRIPTED PROMPT SELECTED] Prompt:', prompt);
  console.log('[SCRIPTED PROMPT SELECTED] Source: getIntakeStageTextSafe()');
  console.log('[SCRIPTED PROMPT SELECTED] Timestamp:', new Date().toISOString());
  console.log('[SCRIPTED PROMPT SELECTED] =========================================');
...
function handleSimpleModeConnection(ws: WebSocket, req: any) {
  ...
  const persistPartialIntake = async (stage: string, field: string) => {
    ...
    try {
      ...
      // Update lead metadata with partial intake metadata only
      ...
      const { error: metaUpdateError } = await supabase
        ...
        .update({
          raw_metadata: {
            ...
            ai_intake_partial_call_sid: state.callSid,
          }
        })
        .eq('id', lead.id);
  ...
  const logFieldCapture = (field: string, rawTranscript: string | undefined, extractedValue: string, accepted: boolean, reason: string, storedValue: string) => {
    console.log('[FIELD CAPTURE] =========================================');
    console.log('[FIELD CAPTURE] field:', field);
    console.log('[FIELD CAPTURE] rawTranscript:', rawTranscript || '');
    console.log('[FIELD CAPTURE] extractedValue:', extractedValue || '');
    console.log('[FIELD CAPTURE] accepted:', accepted);
    console.log('[FIELD CAPTURE] reason:', reason);
    console.log('[FIELD CAPTURE] storedValue:', storedValue || 'Not collected');
    console.log('[FIELD CAPTURE] =========================================');
  };
  // ─────────────────────────────────────────────────────────────────────────
  ...
  const processSimpleModeCompletion = async () => {
    ...
    const cleanNameMinimal = (candidate: string): string => {
      ...
      // If name contains a comma followed by request phrase, keep only text before comma
    ...
    // Minimal address cleanup: remove obvious conversational prefixes only
    const cleanAddressMinimal = (candidate: string): string => {
      if (!candidate) return '';
      let cleaned = candidate.trim();
      // Remove only the most obvious conversational prefixes
      cleaned = cleaned.replace(/^(?:it'?ll be|it'?s at)\s*/i, '');
    ...
    const parseNameAndService = (text: string, existingService?: string): { customerName: string; serviceRequested: string } => {
      ...
      if (existingService) {
        ...
        const nameCandidate = serviceIdx > 0
          ...
          : trimmed;
        customerName = normalizeNameCandidate(nameCandidate) || trimmed;
        parserRuleMatched = 'existing_service';
        console.log('[NAME PARSE] =========================================');
        console.log('[NAME PARSE] rawInput:', trimmed);
        console.log('[NAME PARSE] normalizedInput:', trimmed);
        console.log('[NAME PARSE] parsedName:', customerName);
        console.log('[NAME PARSE] parsedReason:', serviceRequested);
        console.log('[NAME PARSE] parserRuleMatched:', parserRuleMatched);
        console.log('[NAME PARSE] =========================================');
        return { customerName, serviceRequested };
      }
    ...
    const normalizeCrmField = (
      ...
      logFieldCapture(fieldName, value, value, true, 'accepted_spoken_answer_verbatim', stored);
      return stored;
    };
    // ─────────────────────────────────────────────────────────────────────────
        ...
        state.openAiWs.on('message', (data) => {
          ...
          if (message.type === 'conversation.item.input_audio_transcription.completed') {
            ...
            if (fieldName) {
              ...
              console.log('[NAME_ONLY_TRACE] authorizedTurnId:', authorizedTurnId);
              console.log('[NAME_ONLY_TRACE] customerNameAfter:', state.intakeData.customerName);
              ...
              console.log('[ASK_NAME_REASON TURN MERGED] authorizedTurnId:', authorizedTurnId);
            ...
            if (accepted && !state.assistantSpeaking) {
              ...
              if (state.currentStage === 'ask_name_reason') {
                ...
                if (hasValidCustomerName && hasValidServiceRequested) {
                  ...
                  console.log('[NAME_ONLY_TRACE] selectedPromptKey:', promptKeyOverride || state.currentStage);
...
SPEAK ONLY the exact text provided by the app via response.create instructions.`
            ...
            openAiWs.on('message', async (data) => {
              ...
              if (message.type === 'response.created') {
                ...
                console.log('[ASSISTANT SPEAKING TRUE - BEFORE RESPONSE CREATE] Response ID:', responseId);
              ...
              if (message.type === 'response.audio.done') {
                ...
                console.log('[ASSISTANT SPEAKING FALSE - PLAYBACK COMPLETE] Response ID:', message.response_id || 'unknown');
                console.log('[ASSISTANT SPEAKING FALSE - PLAYBACK COMPLETE] Item ID:', message.item_id || 'unknown');
...
async function generateSingleCachedAudio(prompt: string): Promise<string | null> {
  try {
    ...
    if (!response.ok) {
      console.error('[ADMIN] OpenAI TTS API error:', response.status, response.statusText);
      return null;
    }

    const arrayBuffer = await response.arrayBuffer();
    const pcmBuffer = Buffer.from(arrayBuffer);
    console.log(`[ADMIN] Received ${pcmBuffer.length} bytes of PCM audio`);
    ...
    console.log(`[ADMIN] Converted to ${mulawBuffer.length} bytes of μ-law audio`);

    return mulawBuffer.toString('base64');
  } catch (error) {
    console.error('[ADMIN] Error generating audio:', error);
    return null;
  }
}
