...
export default function TapToPayModal({
  ...
  if (showReceiptModal && typeof document !== 'undefined') {
    return createPortal(
      <div className="fixed inset-0 z-[70] flex items-center justify-center p-3 sm:p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" style={{ paddingTop: 'max(16px, env(safe-area-inset-top))', paddingBottom: 'max(16px, env(safe-area-inset-bottom))' }}>
        <div className="bg-card rounded-2xl shadow-2xl shadow-black/10 dark:shadow-black/30 border border-border/50 w-full max-w-sm max-h-[var(--modal-max-height)] overflow-hidden flex flex-col min-h-0 animate-in zoom-in-95 duration-200">
