...
import { CreditCard, CheckCircle2, XCircle, Clock, AlertCircle, ArrowDown } from 'lucide-react'
      ...
      {conversationTimeline.map((item: any, index: number) => {
        ...
        if (item.type === 'payment_requested') {
          ...
          return (
            ...
            >
              <div className="flex flex-col items-center gap-2 bg-muted/30 px-4 py-3 rounded-md border border-border/20 shadow-sm max-w-[85%]">
                ...
                {firstNonPlaceholder(payment.display_name, payment.description) && (
                  <div className="text-xs text-foreground/70">
                    For: {firstNonPlaceholder(payment.display_name, payment.description)}
                  </div>
                )}
