...
export default function LeadsSourceGraph() {
  ...
  const [activeSlice, setActiveSlice] = useState<LeadSourceData | null>(null)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const { chartData, unclassifiedCount: localUnclassifiedCount } = normalizeSourceCounts(sourceCounts)
  ...
  return (
    <Card className="h-full" variant="hero" padding="md">
      <div className="p-4 sm:p-5">
        <ChartHeaderControls title="Leads by Source">
          <ChartFilterButton
            groups={[{
              label: 'Time range',
              value: timeRange,
          ...
          <div className="mt-1 h-7 flex items-center justify-center">
            {activeSlice && (
              ...
              >
                ...
                <span className="text-muted-foreground/70">{Math.round((activeSlice.value / trueTotal) * 100)}%</span>
              </button>
            )}
          </div>
          </>
