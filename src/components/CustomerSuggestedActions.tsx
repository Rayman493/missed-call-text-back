...
import { generateCustomerSuggestedActions } from '@/lib/suggested-actions/suggested-actions-service'
...
interface CustomerSuggestedActionsProps {
  business: Business | null
  customerId: string
}

export default function CustomerSuggestedActions({ business, customerId }: CustomerSuggestedActionsProps) {
  ...
  useEffect(() => {
    async function fetchSuggestedActions() {
      ...
      try {
        ...
        const insights = await generateCustomerInsights(business.id, customerId, supabase)
        const actions = generateCustomerSuggestedActions(insights)
        ...
        {suggestedActions.map((action) => (
          <div key={action.id}>
            {action.outcome === 'completed' ? (
              ...
              >
                <div className="flex items-start gap-2 p-2 rounded-md hover:bg-muted/20 transition-colors group">
