...
export default function SettingsContent({ section }: { section?: string } = {}) {
  ...
  useEffect(() => {
    const handleHash = () => {
      ...
      if (hash === 'out-of-office') {
        ...
        // Wait for section expansion and then scroll
        setTimeout(() => {
  ...
  // Never returns undefined, null, empty string, whitespace-only, or literal "undefined" values
  ...
  // Normalize broken default templates that contain literal "undefined" or "null"
  const normalizeBrokenTemplates = (message: string | null | undefined): string => {
    if (!message) return ''
  ...
  const getFollowUpSettings = () => {
    ...
    const sourceSettings = formBusiness?.automation_settings || business?.automation_settings
    if (!sourceSettings || !sourceSettings.followUps) {
      return defaults
    }

    // Normalize any broken templates in saved settings
    const normalizedFollowUps = (sourceSettings.followUps.followUps || defaults.followUps).map((followUp: any) => ({
      ...followUp,
      message: normalizeBrokenTemplates(followUp.message)
    }))

    return { enabled: sourceSettings.followUps.enabled ?? true, followUps: normalizedFollowUps }
  }

  // Helper to update follow-up settings in automation_settings
  const updateFollowUpSettings = (updates: { enabled?: boolean; followUps?: any[] }) => {
    const currentSettings = getFollowUpSettings()
    ...
    updateBusiness({
      automation_settings: {
        ...(formBusiness?.automation_settings || business?.automation_settings || {}),
        followUps: {
          enabled: updatedSettings.enabled,
          followUps: updatedSettings.followUps
        }
      }
    })
  }
                ...
                <div className="space-y-5">
                        ...
                        <div className="space-y-2">
                          ...
                          <details className="group rounded-md border border-slate-200/60 dark:border-slate-700/50 bg-slate-50/70 dark:bg-slate-800/30 px-3 py-2">
                            ...
                            <div className="mt-2 space-y-1.5 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                              <p>A dedicated business number is recommended for the best ReplyFlow experience.</p>
                              <p>You can also use a personal number with call forwarding. Personal voicemails are kept separate from customer conversations.</p>
                            </div>
                          </details>
                        </div>
                      )}
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed mb-2">
                      Choose how you typically serve customers. ReplyFlow uses this to ask callers the right questions.
                    </p>
                    <div role="radiogroup" className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {[
                        ...
                        const isSelected = ((formBusiness as any).service_location_type || 'onsite') === opt.value
                        return (
                          <button
                            key={opt.value}
                            type="button"
                            ...
                            className={`text-left p-3 rounded-md border transition cursor-pointer ${
                              isSelected
                                ? 'border-blue-500 bg-blue-50/40 dark:bg-blue-900/20'
                                : 'border-slate-200/70 dark:border-slate-700/50 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50/50 dark:hover:bg-slate-800/30'
                            }`}
                          >
                              ...
                              <div key={followUp.step} className="border border-border/30 rounded-lg p-3">
                                <div className="flex items-start justify-between mb-3">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <button
                                      onClick={() => {
                                        const updatedFollowUps = getFollowUpSettings().followUps.map((fu: any) =>
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <Dropdown
                                        ...
                                        onChange={(value) => {
                                          const updatedFollowUps = getFollowUpSettings().followUps.map((fu: any) =>
                                            fu.step === followUp.step ? { ...fu, delayUnit: value } : fu
                                          )
                                          updateFollowUpSettings({ followUps: updatedFollowUps })
                                        }}
                                        disabled={!followUp.enabled}
                                        size="sm"
                                        className="w-32"
                                        aria-label={`Follow-up ${followUp.step} time unit`}
                                      />
                                    </div>
                                  </div>
                                  ...
                                  <div>
                                    ...
                                    <textarea
                                      value={normalizeBrokenTemplates(followUp.message)}
                                      onChange={(e) => {
                                        const updatedFollowUps = getFollowUpSettings().followUps.map((fu: any) =>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                          {(() => {
                            ...
                            if (!stripeChargesEnabled) {
                              return 'Connect Stripe above to enable contactless payments.'
                            }

                            return 'Accept contactless payments directly on your iPhone.'
                          })()}
                        </p>
                      </div>
                    ...
                    <div className="mt-auto space-y-2">
                      {tapToPayAwareness.state.isLoading ? (
                        ...
                        (() => {
                          ...
                          if (!stripeChargesEnabled) {
                            return null // Already handled in main description
                          }
                          ...
                          if (status === 'unsupported_device' && unsupportedReason === 'unsupported_device_type') {
                            ...
                            if (deviceType === 'ipad') {
                              return (
                                <div className="text-xs text-slate-500 dark:text-slate-400">
                                  Use a compatible iPhone instead.
                                </div>
                              )
                            }
                            return (
                  ...
                  <div id="payments-paypal" className="flex flex-col h-full border border-border/30 rounded-lg p-3 sm:p-4">
                    <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3 mb-2">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                          Accept PayPal payments by username.
                        </p>
                      </div>
                    </div>
                    <div className="mt-auto">
                      <label className="block text-xs font-medium text-foreground mb-1">PayPal username</label>
                      <input
                        type="text"
