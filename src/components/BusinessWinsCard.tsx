    ...
    const fetchAchievements = async () => {
      try {
        ...
        const [
          ...
          // Calendar-linked jobs with error handling
          ...(await (async () => {
            try {
              const result = await supabase.from('jobs').select('*', { count: 'exact', head: true }).eq('business_id', business.id).not('google_calendar_event_id', 'is', null)
              return [result]
            } catch (error: any) {
              console.error('[BusinessWinsCard] Calendar-linked jobs count query failed', {
                code: error?.code,
          ...
          // Calendar-linked jobs first job with error handling
          ...(await (async () => {
            try {
              const result = await supabase.from('jobs').select('created_at').eq('business_id', business.id).not('google_calendar_event_id', 'is', null).order('created_at', { ascending: true }).limit(1).maybeSingle()
              return [result]
            } catch (error: any) {
              console.error('[BusinessWinsCard] Calendar-linked jobs first job query failed', {
                code: error?.code,
                message: error?.message,
                details: error?.details,
                hint: error?.hint,
