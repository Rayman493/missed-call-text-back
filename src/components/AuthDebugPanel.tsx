...
export function AuthDebugPanel() {
  ...
  const loadEvents = () => {
    if (typeof window !== 'undefined') {
      try {
        const stored = sessionStorage.getItem('authDebugEvents')
        if (stored) {
  ...
  const addEvent = (type: string, data: any) => {
    ...
    setEvents(prev => {
      ...
      if (typeof window !== 'undefined') {
        try {
          sessionStorage.setItem('authDebugEvents', JSON.stringify(updated))
        } catch (error) {
  ...
  const clearEvents = () => {
    setEvents([])
    if (typeof window !== 'undefined') {
      sessionStorage.removeItem('authDebugEvents')
    }
  }
  ...
  return (
    <div className="fixed top-4 right-4 w-96 max-h-96 bg-black text-green-400 font-mono text-xs border border-green-600 rounded-lg shadow-2xl z-50 overflow-hidden">
      <div className="bg-green-900 text-green-100 p-2 border-b border-green-600">
        <div className="flex justify-between items-center">
          <h3 className="text-sm font-bold">Auth Debug Panel</h3>
