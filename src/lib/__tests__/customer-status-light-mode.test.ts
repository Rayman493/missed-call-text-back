...
describe('Customer Status Light Mode Contrast', () => {
  describe('Card surface contrast in light mode', () => {
    it('should have stronger light-mode surface opacity for new status', () => {
      ...
      expect(style.cardClass).toContain('from-blue-50/80')
      expect(style.cardClass).toContain('via-blue-50/70')
      expect(style.cardClass).toContain('border-blue-500/60')
    })

    it('should have stronger light-mode surface opacity for active status', () => {
      const style = CUSTOMER_STATUS_STYLES.active
      expect(style.cardClass).toContain('from-green-50/80')
      expect(style.cardClass).toContain('via-green-50/70')
      expect(style.cardClass).toContain('border-green-500/60')
    })

    it('should have stronger light-mode surface opacity for completed status', () => {
      const style = CUSTOMER_STATUS_STYLES.completed
      expect(style.cardClass).toContain('from-slate-100/80')
      expect(style.cardClass).toContain('via-slate-100/70')
      expect(style.cardClass).toContain('border-slate-400/60')
    })
  ...
  describe('All statuses use consistent light-mode pattern', () => {
    ...
    it('should have increased light-mode opacity for all statuses', () => {
      statuses.forEach(status => {
        ...
        // All should have increased light-mode opacity (80/70 instead of 50/60)
        expect(style.cardClass).toMatch(/\/80|\/70/)
        // All should have increased border opacity (/60 instead of /50)
        expect(style.cardClass).toMatch(/\/60/)
