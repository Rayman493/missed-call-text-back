    ...
    if (!customerId) {
      ...
      if (!followUpsToday || followUpsToday.length === 0) {
        insights.push({
          ...
          description: 'Check for customers who may need attention',
          actionable: true,
          confidence: 70,
    ...
    if (customerId) {
      ...
      followUpJobs?.forEach((job: any) => {
        ...
        insights.push({
          ...
          category: 'operations',
          priority,
          title: isOverdue ? 'Follow-up is overdue' : 'Follow-up scheduled',
          description: isOverdue ? 'A follow-up was scheduled for a past date' : 'A follow-up is scheduled',
          actionable: true,
          customerId,
          confidence: 85,
