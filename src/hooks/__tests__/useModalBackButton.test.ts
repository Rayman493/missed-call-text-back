...
import {
  ...
  consumeHistoryBackSuppression,
} from '@/lib/modalBackButton'
...
describe('useModalBackButton — navigation-driven close suppression', () => {
  beforeEach(() => {
    while (getModalStack().length > 0) getModalStack().pop()
    consumeHistoryBackSuppression()
    window.history.pushState = vi.fn()
    window.history.back = vi.fn()
  })
  ...
  it('suppressNextHistoryBackCleanup sets the one-shot flag', () => {
    suppressNextHistoryBackCleanup()
    expect(consumeHistoryBackSuppression()).toBe(true)
    expect(consumeHistoryBackSuppression()).toBe(false)
  })

  it('navigation close skips history.back in cleanup', () => {
    registerModal(vi.fn())
    suppressNextHistoryBackCleanup()
    unregisterModal(getModalStack()[0])
