...
function AuthContent() {
  ...
  useEffect(() => {
    ...
    const oauthError = typeof window !== 'undefined' ? sessionStorage.getItem('oauth_error') : null
    if (oauthError) {
      console.log('[Auth] OAuth error from callback:', oauthError)
      setError(oauthError)
  ...
  const handleGoogleOAuth = async () => {
    ...
    try {
      ...
      if (isCapacitorNative()) {
        ...
        console.log('[Auth] Opening Google OAuth in external browser')
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-4 sm:py-8">
        ...
        <div className="w-full max-w-md sm:max-w-[480px] bg-gradient-to-b from-slate-900 to-slate-900/95 border border-slate-700/50 rounded-2xl shadow-xl shadow-blue-900/5 p-5 sm:p-6 md:p-8 backdrop-blur-sm">
          ...
          <button
            ...
            className="w-full h-12 bg-white dark:bg-slate-100 text-slate-900 dark:text-slate-900 py-2 px-4 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg transition-all hover:-translate-y-[1px] font-semibold flex items-center justify-center gap-3 border border-slate-200 dark:border-slate-300"
          >
            {loading ? (
