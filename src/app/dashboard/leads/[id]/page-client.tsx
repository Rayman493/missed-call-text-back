                      ...
                      <SidebarSection
                        ...
                        headerAction={
                          <button
                            ...
                            onClick={() => setShowPaymentModal(true)}
                            disabled={!business || getAvailableProviders(business).length === 0}
                            className="inline-flex items-center gap-2 px-3 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-medium rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/40 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Add
                          </button>
                        }
    ...
    {isFullScreen && typeof document !== 'undefined' && createPortal(
      <div className="fixed inset-0 z-[999] flex items-center justify-center bg-background animate-in fade-in duration-200 p-4 sm:p-8" role="dialog" aria-modal="true" aria-label="Full screen conversation">
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] bg-radial-gradient from-transparent via-muted/20 to-muted/40 dark:via-slate-900/20 dark:to-slate-950/40"></div>
        </div>

        {/* Unified workspace container with subtle boundary */}
        <div className="relative flex flex-col w-full max-w-6xl h-full bg-background/80 dark:bg-slate-900/60 border border-border/30 shadow-2xl animate-in zoom-in-95 duration-200 ease-out">
          {/* Header - Substantial elevated surface */}
          <div className="flex items-center justify-between px-6 sm:px-8 py-4 sm:py-5 border-b border-border/40 bg-background/95 dark:bg-gradient-to-b dark:from-slate-900 dark:to-slate-900/95 flex-shrink-0 shadow-sm" style={{ paddingTop: 'calc(env(safe-area-inset-top) + 1rem)' }}>
            <div className="flex items-center gap-2">
        ...
        <div className="shrink-0 border-t border-border/40 bg-background/95 dark:bg-gradient-to-b dark:from-slate-900/80 dark:via-slate-900/70 dark:to-slate-900/60 px-6 sm:px-8 py-4 sm:py-5 shadow-[0_-4px_24px_-4px_rgba(0,0,0,0.08)]" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 0.5rem)' }}>
          {(() => {
            const effectiveSource = (sendingSource === 'business' && supportsBusiness) ? 'business' : 'replyflow'
            if (effectiveSource === 'business') {
              const customerName = getCustomerName(lead, leadData)
