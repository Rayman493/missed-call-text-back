...
export async function POST(
  ...
  try {
    ...
    if (authError || !user) {
      console.error('[PAUSE ALL FOLLOW-UPS ERROR] Authentication failed:', authError)
    ...
    if (leadError || !lead) {
      console.error('[PAUSE ALL FOLLOW-UPS ERROR] Lead not found:', leadError)
      return NextResponse.json({ error: 'Lead not found' }, { status: 404 })
    }

    if (lead.business_id !== business.id) {
      console.error('[PAUSE ALL FOLLOW-UPS ERROR] Lead does not belong to user\'s business', {
        leadId,
