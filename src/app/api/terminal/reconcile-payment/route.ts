...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Only update if PaymentIntent is succeeded
    if (paymentIntent.status !== 'succeeded') {
      console.log('[TERMINAL_RECONCILIATION] not_succeeded_yet status=' + paymentIntent.status)
      return NextResponse.json({
        status: 'pending',
        paymentRequestId: paymentRequest.id,
      })
    ...
    console.log('[TERMINAL_RECONCILIATION] stage=update_to_paid')
    ...
    console.log('[TERMINAL_RECONCILIATION] update_success=true')
    ...
    console.log('[TERMINAL_RECONCILIATION] stage=complete')
