...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!lookupResult.found || lookupResult.reason !== 'found' || !lookupResult.business) {
      console.error('[NOTIFICATION MARK ALL READ] Unauthorized: User does not have a business', {
        userId: user.id,
    ...
    if (lookupResult.business.id !== businessId) {
      console.error('[NOTIFICATION MARK ALL READ] Unauthorized: User cannot access this business', {
        userId: user.id,
