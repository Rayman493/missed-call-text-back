...
ADD COLUMN sms_scheduled_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN sms_sent_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN sms_message_sid VARCHAR(255);

-- Add index for efficient querying of pending SMS
CREATE INDEX idx_call_events_sms_pending ON call_events(sms_pending, twilio_call_sid);
