-- Repair billing document numbering: ensure tables, function, RLS, policies,
-- triggers, and grants exist with the canonical definitions.
--
-- This migration is IDEMPOTENT and safe to run whether or not the original
-- foundation migration (20260913210000_create_billing_documents.sql) was
-- applied. It uses CREATE TABLE IF NOT EXISTS, CREATE OR REPLACE FUNCTION,
-- DROP ... IF EXISTS, and CREATE ... IF NOT EXISTS throughout.
--
-- Root cause addressed: the foundation migration was not applied to production
-- (only the logo-fix migrations were manually run), so the
-- assign_billing_document_number RPC was missing, causing PGRST202 errors
-- when saving Quote/Invoice drafts.
...
-- Uses a counter table + row lock for concurrency-safe assignment.
-- ---------------------------------------------------------------------------
...
--
-- We DROP first (IF EXISTS) then CREATE to handle the case where the
-- function exists with a different signature or missing SECURITY DEFINER
-- / search_path attributes from the original (pre-rename) migration.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- updated_at triggers (drop + recreate for idempotency)
-- ---------------------------------------------------------------------------
DROP TRIGGER IF EXISTS billing_documents_updated_at_trigger ON billing_documents;
CREATE TRIGGER billing_documents_updated_at_trigger
    BEFORE UPDATE ON billing_documents
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS billing_document_items_updated_at_trigger ON billing_document_items;
...
DROP POLICY IF EXISTS "billing_documents_insert_own" ON billing_documents;
...
DROP POLICY IF EXISTS "billing_documents_update_own" ON billing_documents;
...
DROP POLICY IF EXISTS "billing_documents_delete_own" ON billing_documents;
...
DROP POLICY IF EXISTS "billing_document_items_select_own" ON billing_document_items;
...
DROP POLICY IF EXISTS "billing_document_items_insert_own" ON billing_document_items;
...
DROP POLICY IF EXISTS "billing_document_items_update_own" ON billing_document_items;
...
DROP POLICY IF EXISTS "billing_document_items_delete_own" ON billing_document_items;
...
DROP POLICY IF EXISTS "billing_document_counters_select_own" ON billing_document_counters;
